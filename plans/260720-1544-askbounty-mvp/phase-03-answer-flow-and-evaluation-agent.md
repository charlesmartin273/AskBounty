...
- **M3 — fail-closed eval verdict:** schema-validate `callLlm` output (field checks / whitelist status values); ANY parse or validation failure = evaluation failed, never accepted.
- **Eval provider = Gemini free tier (PRD-ERRATA E5):** use `lib/eval/llm-client.ts` (`callLlm` + `VERDICT_SCHEMA`). On `LlmEvalError` (429/401/timeout/5xx/bad output): answer status → "evaluation pending, retrying" with a MANUAL RETRY button; auto-retry only when `error.retryable`; never hang silently, never map failure to accepted.
- **Phase 2 review carry-overs (M2 + lows):** Phase 3 migration must add `ALTER TABLE questions ALTER COLUMN status SET DEFAULT 'draft'` + `CHECK (status IN ('draft','open','answered','expired'))` (schema default 'open' could bypass the live guard on a future insert path). When reading `questions.budget` for payout math, select `budget::text` to avoid float precision at 15+ digits. Answer-side reads of `fund_tx` are informational only — never derive payout amounts from it.
